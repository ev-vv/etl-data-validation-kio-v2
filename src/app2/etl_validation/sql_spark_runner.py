...
ALL_SQL_CHECKS = {
    "E": {
        "stg_schema_matches_key_missing": "SELECT COUNT(*) FROM etl_kio.matches WHERE 1=0",
    ...
    "T": {
        ...
        """,
        "dds_matchday_out_of_range": "SELECT COUNT(*) FROM etl_kio.matches WHERE matchday IS NOT NULL AND (matchday < 0 OR matchday > 60)",
        # Completeness / rules
...
def run_stage_validation_sql_spark(
    *,
    spark: SparkSession,
    dag_id: str,
    stage: str,
    ...
    output_dir = tool_output_dir(output_dir, "sql_spark")
    output_dir.mkdir(parents=True, exist_ok=True)

    checks = ALL_SQL_CHECKS.get(stage, {})
    reports = []
    ...
    for name, sql in checks.items():
        ...
        try:
            ...
            status = "FAIL"
            failed_rows = -1
            logger.warning("SQL Spark check %s failed: %s", name, e)
    ...
    report = {
        "run_id": run_id, "stage": stage, "status": overall,
        "checks_total": len(checks), "checks_failed": total_failed,
        "spark_sec": round(spark_sec_total, 3),
        "results": check_results,
    }
    out_path = output_dir / f"sql_spark_{stage.lower()}_{ts}.json"
