...
- **Spark-контур:** PySpark `local[*]` + Spark SQL/Catalyst поверх таблиц Spark
  в базе `etl_kio`; в этом контуре отдельный MART не материализуется,
  L-этап проверяется на тех же таблицах Spark.
